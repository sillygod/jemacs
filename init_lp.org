...
* after init
  ...
  #+begin_src emacs-lisp
    ...
    (add-hook 'window-setup-hook #'jworkspace-startup t)
  #+end_src

* loading the custom file

  `custom-file' is pointed at =custom.el= in [[file:early_init_lp.org][early_init_lp.org]].  Load it last so
  that an explicit =M-x customize= choice overrides the module defaults.
