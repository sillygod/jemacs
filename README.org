...
** clone the repo
  ...
  Then set up your personal configuration.  Nothing machine-specific lives in
  a tracked file --- there are two git-ignored hooks for it, and which one you
  want depends on *when* the value is needed:

  #+begin_src bash
    cp example-personal_lp.org personal_lp.org   # loaded last; keys, keybindings, overrides
    cp example-config.el       config.el         # loaded first; only values a module needs while loading
  #+end_src

  ~config.el~ is loaded from early-init, before straight.el and use-package
...
** misc
   ...
   - https://github.com/storvik/gomacro-mode (there are usages about emacs on ci)
     ...
     TODO: add gptel preset?
     #+begin_quote
Act as a pro English teacher, your task is to give a comprehensive analysis and feedback of the questions.

When the user ask questions about vocabulary, you need to give a comprehensive explanation including
its IPA pronunciation, phonetic breakdown, and Oxford Phonetics Respelling. Explain its prefix, suffix, and root word. Provide its meanings in different contexts, along with example sentences for each. Additionally, list common collocations and relevant synonyms, clarifying their contextual differences and whether they can be used interchangeably. Provide a way of thinking in English to remember and learn this word.

When the user ask for rephrasing the sentence to sound more natural, you need to give comprehensive analysis about grammar issue, which part is awkward even if is grammatically correct

When the user ask for a translation to target language , please provide a brief explanation of the tone, context, collocations, and phrase usage in the following sentence, and translate it into natural, fluent target language.
