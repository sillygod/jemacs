...
* rime-emacs
  ...
  #+begin_src bash :tangle no
    ...
    tar -xf rime.tar.bz2 -d ~/.emacs.d/librime
  #+end_src
...
** +hydra+ motion (replaced with the builtin functions of transient)
   ...
   #+begin_src emacs-lisp
     ...
     (transient-define-prefix org-roam-transient ()
       ...
       :transient-suffix 'transient--do-stay
       [["Action"
         ("a" "add alias for node" (lambda () (interactive) org-roam-alias-add))
         ;; this is used for whening the name is conflict. It mostly happens in the header name
         ("c" "create node" (lambda () (interactive) org-id-get-create))
         ("i" "insert" (lambda () (interactive) org-roam-node-insert))
         ("f" "find file" (lambda () (interactive) org-roam-node-find))
         ("d" "dailies" (lambda () (interactive) org-roam-dailies-goto-today))
         ("l" "back link buffer" (lambda () (interactive) org-roam-buffer-toggle))
         ("g" "graph" (lambda () (interactive) my-org-roam-ui-open))
         ("r" "db refresh" (lambda () (interactive) my-refresh-org-roam-db-cache))
         ("t" "add tag" (lambda () (interactive) org-roam-tag-add))
         ("q" "quit" transient-quit-all)]])
