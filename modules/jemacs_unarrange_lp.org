...
* counsel-jq-yq
  ...
  #+begin_src emacs-lisp :tangle no
    (use-package jq-yq
      :defer 1
      :straight (
                 :local-repo (concat home-dir "local/jq-yq")
                 )
...
* yasnippet
  ...
  ivy-yasnippet is deprecated because ivy is replaced by consult
  #+begin_src emacs-lisp :tangle no
...
** experiment tree-sitter
   ...
   now tree sitter are builtin in emacs 29
...
* language server protocol mode
  ...
  #+begin_src emacs-lisp
    (defun add-xref-dumb-jump-before-xref-lsp (arg)
    ...
    (use-package eglot
      ...
      (add-hook 'python-mode-hook (lambda ()
                                    (setq-default eglot-workspace-configuration
                                                `((:python .
                                                          (:venvPath ,(poetry-get-virtualenv)
                                                           :pythonPath ,(concat (poetry-get-virtualenv) "/bin")))))
                                    (eglot-ensure)))

      (add-hook 'eglot-managed-mode-hook (lambda () (add-hook 'xref-backend-functions #'dumb-jump-xref-activate nil t)))
  ...
  #+begin_src emacs-lisp :tangle no
        (use-package lsp-mode
          ...
          (setq lsp-completion-provider :none) ;; the official recommends use this
          ...
          :config
          (advice-add #'xref--create-fetcher :override #'my-xref--create-fetcher)
          (advice-add 'lsp-managed-mode :before 'add-xref-dumb-jump-before-xref-lsp))
