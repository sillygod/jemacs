...
* ecloud
  ...
  Deliberately *not* tangled from here.  ecloud is wholly personal --- it is
  pulled from a private checkout and every account maps to a cloud service
  account credential --- so there is nothing here another person could reuse.
  It lives in =personal_lp.org= instead (see =example-personal_lp.org= for the
  shape).  The declaration below is kept only as documentation:
  ...
  Working on a =:local-repo= package means straight caches a build of it.  To
  pick up edits, drop that cache (and the native-comp one) and restart:

  #+begin_src emacs-lisp :tangle no
    (delete-directory (expand-file-name "straight/build/PACKAGE" user-emacs-directory) t)

    (when (boundp 'native-comp-eln-load-path)
      (dolist (dir native-comp-eln-load-path)
        (when (file-directory-p dir)
          (dolist (f (directory-files dir t "PACKAGE"))
            (delete-file f)))))
  #+end_src
...
* docker
  #+begin_src emacs-lisp
    ...
    ;; Emacs 29+: /docker: /podman: /kubernetes: live in tramp-container.
    ;; docker-tramp is obsolete and prints a warning if loaded.
...
* flycheck mode
  ...
  #+begin_src emacs-lisp
    (use-package flycheck
      ...
      :config
      ;; The elisp checkers spawn `emacs -Q --batch', which skips
      ;; early-init.el, so native-comp trampolines (e.g. from
      ;; doom-modeline advising `rename-buffer') would land in
      ;; ~/.emacs.d/eln-cache.  --init-directory keeps them in emacs-home.
      (setq flycheck-emacs-args
            (append flycheck-emacs-args
                    (list (concat "--init-directory=" user-emacs-directory))))
...
* common lisp or emacs lisp
  ...
  #+begin_src emacs-lisp
    ...
    (eval `(use-package lisplet
             :straight (:local-repo ,(concat home-dir "local/lisplet")
                                    :files ("lisplet.el"))
             :hook ((common-lisp-mode . lisplet-mode)
                    (emacs-lisp-mode . lisplet-mode)
                    (scheme-mode . lisplet-mode))))
...
* general
  ...
  #+begin_src emacs-lisp
    (use-package general
      ...
      (with-eval-after-load 'org-shelf
        (transient-define-prefix org-shelf-transient ()
          "Org-shelf note operations."
          :transient-suffix 'transient--do-exit
          [["Action"
            ;; create node: used when a heading name conflicts; file-level
            ;; notes already have an ID in the PROPERTIES drawer.
            ("c" "create node" org-shelf-id-get-create)
            ("i" "insert" org-shelf-insert)
            ("f" "find file" org-shelf-find)
            ("n" "new note" org-shelf-new)
            ("d" "dailies" org-shelf-dailies-goto-today)
            ("l" "backlinks" org-shelf-backlinks)
            ("r" "refresh" org-shelf-refresh)
            ("t" "add tag" org-shelf-tag-add)
            ("<escape>" "quit" transient-quit-all)
            ("q" "quit" transient-quit-all)]
           ]))
      ...
      (transient-define-prefix buffer-transient ()
        ...
        [["Move"
          ...
          ("t" "terminals" switch-terminal :transient transient--do-exit)
          ("<escape>" "quit" transient-quit-all)
          ("q" "quit" transient-quit-all)]
         ["Action"
      ...
      (with-eval-after-load 'org
        (define-leader-key-map-for 'org-mode-map
                                   ...
                                   "r" "org-shelf transient" 'org-shelf-transient
      ...
      (define-leader-key-global
       ...
       "'" 'new-terminal
       "TAB" 'terminal-perform-last-command
       "?" 'describe-bindings)
      ...
      (define-leader-key-global
       ...
       "ah" '(:ignore t :which-key "ghostherd")
       "ahn" '(ghostherd-new :which-key "new agent")
       "ah2" '(ghostherd-new-pair :which-key "new pair (split)")
       "ahs" '(ghostherd-switch :which-key "switch")
       "ahb" '(ghostherd-sidebar :which-key "sidebar")
       "ah." '(ghostherd-next-blocked :which-key "next blocked")
       "ahm" '(ghostherd-message-interactive :which-key "message agent")
       "ahi" '(ghostherd-prompt-interactive :which-key "prompt agent")
       "ahk" '(ghostherd-kill :which-key "kill")
       "ahr" '(ghostherd-rename :which-key "rename")
       "ahl" '(ghostherd-log :which-key "herd log")
       "ah/" '(ghostherd-memory-search :which-key "search memory")
       "ahv" '(ghostherd-memory-view :which-key "view memory")
       "ahI" '(ghostherd-memory-import :which-key "import memory")
       ...
       "aor" '(org-shelf-transient :which-key "org-shelf")
       "aos" '(org-secrets-sidebar :which-key "secrets overlay")
       "aoc" '(org-secrets :which-key "secrets copy")
       "aon" '(org-secrets-new :which-key "secrets new")
       "aog" '(:ignore t :which-key "goto")
       "aogj" '((lambda () (interactive) (find-file (expand-file-name "journal" jemacs-org-root))) :which-key "journal note")
       "aogt" '((lambda () (interactive) (org-file-show-headings (expand-file-name "todo.org" (or jemacs-org-roam-directory jemacs-org-root)))) :which-key "todo note"))
      ...
      (define-leader-key-global
       ...
       "bt" '(switch-terminal :which-key "switch terminal")
      ...
      (define-leader-key-global
       ...
       "lb" '(jworkspace-switch-to-buffer :which-key "switch buffer")
      ...
      (define-leader-key-global
       ...
       "pc" '(ghostel-project-cmd-run :which-key "command overlay"))
      ...
      (define-leader-key-global
       ...
       "gbl" '(blamer-show-commit-info :which-key "this line")
       "gbb" '(magit-blame-addition :which-key "this buffer")
       "gs" '(magit-status :which-key "magit status")
       "gp" '(pr-view :which-key "PR view")
      ...
      (define-leader-key-global
       ...
       "xs" '(send-text-and-move-to-project-terminal :which-key "send content to and focus on terminal"))
     ...
     (with-eval-after-load 'transient
       ...
       (transient-define-prefix mark-transient ()
         ...
         [["Match"
          ...
          ["Misc"
           ("t" "send to terminal" send-text-and-move-to-project-terminal :transient transient--do-exit)
           ]
          ])
