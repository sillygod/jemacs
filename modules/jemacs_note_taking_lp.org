...
* org
  ...
  #+begin_src emacs-lisp
    ...
    (defun my-todo-agenda ()
      ...
      (interactive)
      (org-ql-search
        ;; (expand-file-name "todo.org" org-roam-directory)
        (directory-files (or (bound-and-true-p org-shelf-directory)
    ...
    (use-package org
      ...
      (setq-default org-default-notes-file
                    (expand-file-name "todo.org" jemacs-org-root))
...
* org-shelf
  ...
  Homemade replacement for org-roam: same =:ID:= / =#+title= / =#+filetags=
  headers, in-memory index, no sqlite.  =org-shelf-complete-everywhere=
  wraps a matching title into =[[id:UUID][title]]= while typing (Corfu).
  There is no graph UI.

  #+begin_src emacs-lisp
    (eval `(use-package org-shelf
             :straight (:local-repo ,(concat home-dir "local/org-shelf")
                                    :files ("org-shelf.el"))
             :after org
             :custom
             ;; Read, never hardcode: `:custom' is `customize-set-variable',
             ;; which runs unconditionally at load time and would clobber a
             ;; value set in config.el.
             (org-shelf-directory (or jemacs-org-roam-directory jemacs-org-root))
             (org-shelf-completion-everywhere t)
             :config
             (setq org-shelf-dailies-directory "journal/")
             (org-shelf-mode 1)))
  #+end_src
...
* org password manager
  ...
  Successor is =local/org-secrets= (=M-x org-secrets-sidebar=, =SPC a o s=).
  This block stays until that package has been used in anger.

  #+begin_src emacs-lisp
    (defvar pair-list nil) ;; a property list
    (defvar secret-file-path
      (expand-file-name "management/learning.org.gpg" jemacs-org-root))
...
* org-secrets
  ...
  Overlay + consult password manager.  Vault is
  =management/learning.org.gpg= under =jemacs-org-root=.  Extra vaults
  later go in =org-secrets-extra-files=.  The footer shows the current
  vault (or =all=); =s= switches scope.  Closing the overlay (or
  finishing =org-secrets=) drops the in-memory cache; =gpg-agent=
  still owns the passphrase TTL.

  #+begin_src emacs-lisp
    (eval `(use-package org-secrets
             :straight (:local-repo ,(concat home-dir "local/org-secrets")
                                    :files ("org-secrets.el"))
             :commands (org-secrets
                        org-secrets-sidebar
                        org-secrets-new
                        org-secrets-insert-password
                        org-secrets-forget)
             :config
             (setq org-secrets-file
                   (expand-file-name "management/learning.org.gpg"
                                     jemacs-org-root))))
  #+end_src
...
* org configuration
  ...
  #+begin_example
  (setq org-agenda-files (file-expand-wildcards (expand-file-name "*.org" jemacs-org-root)))
  (setq org-agenda-files (directory-files-recursively jemacs-org-root "\\.org$"))
  #+end_example
  ...
  #+begin_src emacs-lisp
    (with-eval-after-load 'org
      ...
      (setq org-journal-dir (expand-file-name "journal/" jemacs-org-root))
      (setq org-journal-file-type 'weekly)
      (setq org-journal-file-format "%Y-%m-%W.org")

      (setq org-agenda-files
            (seq-remove (lambda (f) (string-match-p "/\\(presentation\\|journal\\)/" f))
                        (directory-files-recursively
                         (expand-file-name jemacs-org-root) "\\.org\\'")))
