...
* org-roam
  ...
  #+begin_src emacs-lisp
    ...
    (use-package org-roam
      ...
      :after org
      :custom
      ;; Read, never hardcode: `:custom' is `customize-set-variable', which
      ;; runs unconditionally at load time and would clobber a value set in
      ;; config.el.  `org-roam-db-location' is derived here and now, so the
      ;; root has to be right by this point -- overriding it later from
      ;; settings.el would leave the db pointing at the old directory.
      (org-roam-directory (or jemacs-org-roam-directory jemacs-org-root))
