...
* git version control
  ...
  #+begin_src emacs-lisp
    (use-package blamer
      ...
      (setq blamer-bindings '(("s" . blamer-callback-show-commit-diff))))
    ...
    ;; GUI xwidget PR list + description + unified diff (GitHub + Bitbucket Cloud).
    ;; Entry: M-x pr-view / SPC g p.  Tokens stay in Elisp; ui/ only renders.
    (eval `(use-package pr-view
             :straight (:local-repo ,(concat home-dir "local/pr-view")
                                    :files ("pr-view.el" "ui"))
             :commands (pr-view
                        pr-view-github
                        pr-view-bitbucket
                        bitbucket-pr
                        pr-view-open-in-browser
                        pr-view-copy-url)))
  #+end_src
