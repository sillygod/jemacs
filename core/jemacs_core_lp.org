...
** go to dotfile
   #+begin_src emacs-lisp
     ...
     This opens `personal_lp.org', not the `settings.el' it tangles to --- edits
     to the tangle output are thrown away by the next `export-lp-configs'."
       (interactive)
       (find-file-existing (expand-file-name "personal_lp.org" home-dir)))
   #+end_src
...
** org tangle lp configs
   #+begin_src emacs-lisp
     ...
     (defvar org-tangle-cmd (concat emacs-path " --batch --init-directory=" user-emacs-directory " --eval \"(require 'org)\" --eval \"(require 'ob-tangle)\" --eval '(find-file \"%s\")' --eval '(org-babel-tangle)'"))
     ...
     (defun my-encode-region-base64 (start end)
       ...
       (let ((content (buffer-substring-no-properties start end)))
         (when (use-region-p)
           ...
           (insert (base64-encode-string (encode-coding-string content 'utf-8) t)))))
...
** xwidget relevant functions
   #+begin_src emacs-lisp
     (defun rg-query-files (input dir)
       "Run rg --files in DIR and filter with rg --ignore-case INPUT.

       find-name-dired (find is too slow)
       consult-find  (use find)
     "
       ;; (let ((command (concat "rg --files " dir " | rg --ignore-case " (shell-quote-argument input))))
       (let ((command (concat "fd " (shell-quote-argument input) " " dir)))
         (split-string
     ...
     (defun rg-file-search ()
       "Search files or directories in a selected directory using rg with consult."
       (interactive)
       (require 'consult)
         ...
         (find-file
          (consult--read
           (consult--process-collection
               #'(lambda (input) (wrap-to-consult '("printf" "%s\n") #'rg-query-files input selected-dir)))
           :prompt "Search files: "
...
** operation with marked text
   ...
   #+begin_src emacs-lisp
     (defun send-text-and-move-to-project-terminal ()
       "Send the region to the project ghostel terminal and focus it."
       (interactive)
...
** perform the last command in the project terminal

   #+begin_src emacs-lisp
     (defun terminal-perform-last-command ()
       "Replay the previous shell command in the project ghostel terminal."
       (interactive)
