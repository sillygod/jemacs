...
* loading custom settings
  ...
  #+begin_src emacs-lisp
    (when (and (fboundp 'native-comp-available-p)
      ...
      ;; Relative to `user-emacs-directory', which is emacs-home/ above.
      ;; Do not touch `startup--original-eln-load-path': Emacs 31 skips
      ;; its later `startup--update-eln-cache' when the path already
      ;; differs from the original ~/.emacs.d/eln-cache.
      ;;
      ;; `emacs --batch' skips this file (-q), so trampolines from a
      ;; bare batch Emacs still land in ~/.emacs.d/eln-cache.  Batch
      ;; jobs that should use emacs-home must pass
      ;; --init-directory=.../emacs-home/.
    ...
    ;; Anything machine-specific that a module needs *while it is being
    ;; required* has to be declared here with a neutral default, because
    ;; config.el is the only personal file that runs this early.
    ;; See `example-config.el' for how to fill these in.
    (defvar jemacs-local-repo-root nil
      "Directory holding personally checked-out packages, or nil.
Modules that pull a package from a local checkout should read this
instead of hardcoding an absolute path, and should skip the package
entirely when it is nil.")

    (defvar jemacs-org-root (expand-file-name "~/org/")
      "Root directory holding your org files.
Everything under it -- the agenda files, the journal, the notes file --
is derived from this, so only this one value is machine-specific.")

    (defvar jemacs-org-roam-directory nil
      "Directory org-shelf indexes.  Defaults to `jemacs-org-root' when nil.
Set this only if your notes live in a subdirectory of the root.")
...
* custom file
  ...
  =M-x customize= writes into `custom-file', which defaults to whatever
  loaded first -- that used to be =init.el=, a tangle output, so every
  =./lp.sh= run silently threw those settings away.  Point it at a real
  file instead.  It is loaded at the *end* of [[file:init_lp.org][init_lp.org]] so that an explicit
  customization wins over the module defaults.

  #+begin_src emacs-lisp
    (setq custom-file (expand-file-name "custom.el" home-dir))
  #+end_src
